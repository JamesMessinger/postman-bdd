...
module.exports = function SuperAgent () {
  ...
  defineProperties(superAgent.response, {
     ...
     */
    body: function () {
      var body = superAgent.response.text;

      // Does the response looks like a JSON mime type?
      // (e.g. "text/json", "application/hal+json", etc.)
      if (superAgent.response.type.indexOf('json') >= 0) {
        try {
          body = JSON.parse(superAgent.response.text);
        }
        catch (err) {
      ...
      Object.defineProperty(superAgent.response, 'body', {
        configurable: true,
        enumerable: true,
        writable: true,
        value: body,
    ...
    cookies: function () {
      ...
      var header = superAgent.response.getHeader('set-cookie');
